...
int checkAllBoard(Board *b)
{
    ...
    for(i = 0; i < b->size; i++)
    {
        if(aux1 == b->size)
        {
            ...
            aux2 = aux2 + b->squareSize; /* Increments k so that the box starts at the left corner up cell */
        }
        ...
        val = checkValidity(boxes, b->size);
        if(val == FALSE)
        {
            return FALSE;
        }
...
void updateBoardValues(Board *board, int row, int col, int value)
{
    ...
    for(i = 0; i < (board->size); i++)
    {
        /* Update cells in the same row, if the cell is empty and it's not fixed yet */
        if(board->gameBoard[row][i].value == 0 || board->gameBoard[row][i].fixed == FALSE)
        {
        ...
        /* Update cells in the same column, if the cell is empty and it's not fixed yet */
        if(board->gameBoard[i][col].value == 0 || board->gameBoard[i][col].fixed == FALSE)
        {
        ...
        /* Update cells in the same box, if the cell is empty and it's not fixed yet */
        if(board->gameBoard[boxStartRow + i / board->squareSize][boxStartCol + i % board->squareSize].value == 0 || board->gameBoard[boxStartRow + i / board->squareSize][boxStartCol + i % board->squareSize].fixed == FALSE)
        {
